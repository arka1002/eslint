...
module.exports = {
    ...
    create(context) {
         ...
         * @param {any} restrictedProperties they can be either globally restricted or passed with objects
         ...
         */
        function getMatchingProperties(restrictedProperties, propertyNames) {
            const matchedProperties = [];
            propertyNames.forEach(property => {

                if (!restrictedProperties.has(property)) return;

                matchedProperties.push({ name: property, ...restrictedProperties.get(property) })
            });

            return matchedProperties.length === 0 ? undefined : matchedProperties;
        }
        ...
        function checkPropertyAccess(node, objectName, propertyName) {
            ...
            const matchedObjectProperty = matchedObject ? getMatchingProperties(matchedObject, propertyName) : undefined;
        ...
        function checkDestructuringAssignment(node) {
            if (node.right.type === "Identifier" || node.right.type === "CallExpression") {
                ...
                if (node.left.type === "ArrayPattern") {
                    node.left.elements.forEach(node => {
                        if (node?.type === "ObjectPattern") {
                            checkPropertyAccess(node, objectName, astUtils.getAllProperties(node.properties));
                        }
                    })
                }
            ...
            ArrowFunctionExpression: (node) => {
                for (const param of node.params) {
                    if (param.type === "ObjectPattern") {
                        checkPropertyAccess(param, undefined, astUtils.getAllProperties(param.properties))
                    } else if (param.type === "AssignmentPattern" && param.left.type === "ObjectPattern") {
                        checkPropertyAccess(param, param.right?.name, astUtils.getAllProperties(param.left.properties))
                    }
                }
            }
        };
