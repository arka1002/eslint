            ...
            ObjectPattern(node) {
                ...
                if (restrictedProperties.has(objectName)) {
                    node.properties.forEach(property => {
                        ...
                        if (restrictedProperties.get(objectName).has(propertyName)) {
                            const message = restrictedProperties.get(objectName).get(propertyName).message ? ` ${restrictedProperties.get(objectName).get(propertyName).message}` : ``;
                            context.report({
                                node,
                                messageId: "restrictedObjectProperty",
                                data: {
                    ...
                    const message = globallyRestrictedObjects.get(objectName).message ? ` ${globallyRestrictedObjects.get(objectName).message}` : ``;
                    context.report({
                        node,
                        messageId: "restrictedObjectProperty",
                        data: {
                    ...
                    node.properties.forEach(property => {
                        const propertyName = astUtils.getStaticPropertyName(property);
                        if (globallyRestrictedProperties.has(propertyName)) {
                            const message = globallyRestrictedProperties.get(propertyName).message ? ` ${globallyRestrictedProperties.get(propertyName).message}` : ``;
                            context.report({
                                node,
                                messageId: "restrictedProperty",
                                data: {
