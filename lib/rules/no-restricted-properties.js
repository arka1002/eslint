                ...
                if (restrictedProperties.has(objectName)) {
                    /* node.properties.forEach(property => {
                        if (property.key.type === "Identifier" && !property.computed) {
                            if (restrictedProperties.get(objectName).has(property.key.name)) {
                                context.report({
                                    node,
                                    messageId: "restrictedObjectProperty",
                                    data: {
                                        ...
                                        message: restrictedProperties.get(objectName).get(property.key.name).message ? ` ${restrictedProperties.get(objectName).get(property.key.name).message}` : ``
                                    }
                                })
                            }
                            return;
                        }
                        // propertyNames.push(astUtils.getStaticStringValue(property.key));
                        if (restrictedProperties.get(objectName).has(astUtils.getStaticStringValue(property.key))) {
                            context.report({
                                node,
                                messageId: "restrictedObjectProperty",
                                data: {
                                    objectName,
                                    propertyName: astUtils.getStaticStringValue(property.key),
                                    message: restrictedProperties.get(objectName).get(astUtils.getStaticStringValue(property.key)).message ? ` ${restrictedProperties.get(objectName).get(astUtils.getStaticStringValue(property.key)).message}` : ``
                                }
                            })
                        }
                    });  */
                    astUtils.firstOnes(node.properties).forEach(x => {
                        if (restrictedProperties.get(objectName).has(x)) {
                            context.report({
                                node,
                                messageId: "restrictedObjectProperty",
                                data: {
                                    objectName,
                                    propertyName: x,
                                    message: restrictedProperties.get(objectName).get(x).message ? ` ${restrictedProperties.get(objectName).get(x).message}` : ``
                                }
                            })
                        }
                    })
                    ...
                    astUtils.getAllProperties(node.properties).forEach(property => {
                        if (globallyRestrictedProperties.has(property)) {
                            context.report({
