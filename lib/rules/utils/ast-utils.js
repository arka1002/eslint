...
function anotherOne(node) {
    ...
    switch (node && node.type) {
        ...
        case "PropertyDefinition":
        case "MethodDefinition":
            const foo = letsTryToMakeThisWork(node);
            prop = foo;
            break;
    }
    if (prop) {
...
function letsTryToMakeThisWork(node) {
    const array_of_properties = [];
    array_of_properties.push(node.key);
    let unit = node.value;
    function maybeARecursionCanSave(unit) {
        if (unit.type === "ObjectPattern") {
            for (const property of unit.properties) {
                array_of_properties.push(property.key);
                if (property.value.type === "ObjectPattern") {
                    maybeARecursionCanSave(property.value);
                }
            }   
        }
    }
    maybeARecursionCanSave(unit);
    return array_of_properties.length === 1 ? array_of_properties[0] : array_of_properties;
}

function getAllProperties(propArray) {
    const props_used = [];
    function traverse(propArray) {
        for (const node of propArray) {
            if (node.key.type === "Identifier" && !node.computed) {
                props_used.push(node.key.name);
            } else {
                props_used.push(getStaticStringValue(node.key));
            }
